...
public class QuantityBoth extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quantity_both);
    }
    public void sendMessage(View view) {
        ...
        if ((num > 0) && (num <= 10)){
            intent.putExtra(EXTRA_MESSAGE, "rice and dal for " + message + " people");
            Long tsLong = System.currentTimeMillis()/1000;
            String ts = tsLong.toString();
            mDatabase = FirebaseDatabase.getInstance().getReference();
            mDatabase.child("devices").child("device1").child("current_order").child("dish").setValue("both");
